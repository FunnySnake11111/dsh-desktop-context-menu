...
// Electron patches node:fs so that any path ending in ".asar" is treated as an
// asar archive access (readFileFromArchiveSync) — reading the archive itself
// then throws "ENOENT, not found in <archive>". We operate on app.asar's raw
// bytes, so use the unpatched native fs ("original-fs", Electron-only) and
// fall back to node:fs under plain Node.
const require = createRequire(import.meta.url);
let nativeFs = nodeFs;
try {
	const originalFs = require("original-fs");
	if (originalFs && typeof originalFs.readFileSync === "function") nativeFs = originalFs;
} catch { /* plain Node: node:fs is already native */ }
const { copyFileSync, existsSync, readFileSync, writeFileSync, unlinkSync, renameSync, readdirSync, realpathSync } = nativeFs;
...
export function selfHeal({ target: override, logger } = {}) {
	...
	try {
		...
		const detail = error instanceof Error
			...
			: String(error);
		log(`dsh-desktop-context-menu host-patch: failed: ${detail}`);
		if (error instanceof Error && error.stack) log(`dsh-desktop-context-menu host-patch: stack: ${error.stack.split("\n").slice(0, 6).join(" | ")}`);
